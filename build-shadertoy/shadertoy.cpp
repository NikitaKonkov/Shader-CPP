...
// Function to create a ShaderToy-compatible fragment shader
std::string createShaderToyFragmentShader(const std::string& shaderToyCode) {
    char buffer[10000]; // Adjust size as needed
    snprintf(buffer, sizeof(buffer), shaderToyWrapper, shaderToyCode.c_str());
    return std::string(buffer);
}

// Function to render a frame with the ShaderToy shader
void renderShaderToyFrame(ShaderManager& shaderManager, GLuint quadVAO, int width, int height, 
                         float time, float deltaTime, int frame, int mouseX, int mouseY, bool mouseDown) {
    // Clear the screen
